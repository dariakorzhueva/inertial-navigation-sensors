...
public class MadgwickFilter {
    ...
    public MadgwickFilter(float samplePeriod) {
        this(samplePeriod, 1f);
    }

    public MadgwickFilter(float samplePeriod, float beta) {
        mSamplePeriod = samplePeriod;
        mBeta = beta;
    ...
    public void update(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
        ...
        qDot1 = 0.5f * (-q2 * gx - q3 * gy - q4 * gz) - mBeta * s1;
        qDot2 = 0.5f * (q1 * gx + q3 * gz - q4 * gy) - mBeta * s2;
        qDot3 = 0.5f * (q1 * gy - q2 * gz + q4 * gx) - mBeta * s3;
        qDot4 = 0.5f * (q1 * gz + q2 * gy - q3 * gx) - mBeta * s4;

        // Integrate to yield quaternion
        q1 += qDot1 * mSamplePeriod;
        q2 += qDot2 * mSamplePeriod;
        q3 += qDot3 * mSamplePeriod;
        q4 += qDot4 * mSamplePeriod;
